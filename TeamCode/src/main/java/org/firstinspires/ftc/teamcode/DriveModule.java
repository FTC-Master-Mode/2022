...
public class DriveModule {
    ...
    public DriveModule(Robot robot, ModuleSide moduleSide) {
        ...
        if (moduleSide == ModuleSide.RIGHT) {
            motor1 = robot.hardwareMap.dcMotor.get("RightDriveTopMotor");
            motor2 = robot.hardwareMap.dcMotor.get("RightDriveBottomMotorRear");
            positionVector = new Vector2d((double)18/2, 0); //points from robot center to right module
        } else {
            motor1 = robot.hardwareMap.dcMotor.get("LeftDriveTopMotor");
            motor2 = robot.hardwareMap.dcMotor.get("LeftDriveBottomMotor");
